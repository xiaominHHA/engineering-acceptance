...
Flutter 官方骨架最初仅创建 Android 平台，随后使用官方增量命令加入 Web scaffolding：
...
如需 debug APK，可执行 `flutter build apk --debug`。Web release 可执行 `flutter build web --release`；未显式注入 `API_BASE_URL` 时，Web 使用浏览器当前 origin。根目录 `./build.sh` 输出 APK、JAR 和 `dist/frontend/engineering-acceptance-web-<version>/`，并注入 release version 和 Git commit；Android 继续显式注入其 API 地址。
...
根目录统一脚本已实现：`lint.sh` 执行静态检查，`test.sh` 执行隔离 Docker 集成测试，`build.sh` 输出 `dist/` 中的 APK/JAR/Web bundle，`check.sh` 串行执行前三者，`deploy.sh` 只部署显式 release tag。Docker Compose 配置位于 `infra/compose/`。
