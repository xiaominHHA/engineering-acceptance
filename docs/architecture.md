...
- 注册和登录返回短期 HS256 bearer token 与当前用户。token 仅包含 user subject、签发/过期时间，签名密钥来自环境且不进入 Git；服务端保持无状态，不提供 refresh token。
- `GET/PUT /api/users/{id}` 要求 token subject 与 path id 一致，匿名访问返回 401、跨用户访问返回 403。`POST /api/posts` 的作者只取 authenticated principal；`GET /api/posts` 保持匿名可读。
